...
export function typeCastToJava(
	...
	// javaBridge.log.debug('typeCastToJava data:%s', toStr(data));
	// javaBridge.log.debug('typeCastToJava fieldsObj:%s', toStr(fieldsObj));
	...
	traverse(data).forEach(function(value: unknown) { // Fat arrow destroys this
		...
		if (
			...
			// javaBridge.log.debug('pathString:%s', pathString);

			const fieldObjWithoutName = getIn(fieldsObj, pathString, {}) as Required<Omit<DocumentTypeField, 'name'>>;
			// javaBridge.log.debug('fieldObjWithoutName:%s', toStr(fieldObjWithoutName));

			const {
				valueType: valueTypeForPath
			} = fieldObjWithoutName;
			// javaBridge.log.debug(`pathString:${toStr(pathString)} valueTypeForPath:${toStr(valueTypeForPath)}`);
			if (valueTypeForPath) {
				if (arrayIncludes([
					...
					if (isGeoPointArray(value)) {
						try {
							...
							javaBridge.log.error(`isGeoPointArray:true but geoPoint(${toStr(value)}) failed!`, e);
						}
						...
						try {
							...
							javaBridge.log.error(`isGeoPointString:true but geoPointString(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not a geoPoint! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
					...
					if (isDate(value) || isInstantString(value)) {
						try {
							...
							javaBridge.log.error(`isDate or isInstantString is true, but instant(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not an Instant! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
					...
					if (isDate(value) || isLocalDateString(value)) {
						try {
							...
							javaBridge.log.error(`isDate or isLocalDateString is true, but localDate(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not an LocalDate! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
					...
					if (isDate(value) || isLocalDateTimeString(value)) {
						try {
							...
							javaBridge.log.error(`isDate or isLocalDateTimeString is true, but localDateTime(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not an LocalDateTime! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
					...
					if (isDate(value) || isTimeString(value)) {
						try {
							...
							javaBridge.log.error(`isDate or isTimeString is true, but localTime(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not an LocalTime! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
					...
					if (isUuidV4String(value)) {
						try {
							...
							javaBridge.log.error(`isUuid4:true, but reference(${toStr(value)}) failed!`, e);
						}
					} else {
						javaBridge.log.warning(`Not an Reference/UUIDv4! value:${toStr(value)} at path:${pathString} in data:${toStr(data)}`);
					}
				}
			} else { // !valueTypeForPath
				if (!arrayIncludes([FIELD_PATH_GLOBAL, FIELD_PATH_META], pathString)) {
					javaBridge.log.debug(`Field without valueType path:${pathString} value:${toStr(value)}`);
				}
	...
	// javaBridge.log.debug('typeCastToJava typeCastedData %s', typeCastedData);
