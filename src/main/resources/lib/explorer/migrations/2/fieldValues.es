...
import {fieldValue} from '/lib/explorer/model/2/index';


export function fieldValues() {
	log.info('Migrating field values...');
	...
	oldRes.hits.forEach(node => {
		if (!exists({
			...
			node.fieldReference = getReference({
				connection: writeToNewConnection,
				path: dirname(node._path)
			});
	...
	log.info('...done migrating field values.');
} // stopwords
